const squares = document.querySelectorAll(".box");
const turn = document.querySelector(".turn");
const result = document.querySelector(".result");
const button = document.querySelector("button");
const instruct = document.querySelector(".instruct");

button.addEventListener("click", () => {

    result.textContent = "";
    instruct.textContent = "";

    for (let i = 0;i < squares.length; i++){
        squares[i].classList.remove("red");
        squares[i].classList.remove("blue");
    }

    setGame();
});

function setGame () {
    let row1 = document.querySelectorAll(".row1");
        row1 = Array.from(row1);
    let row2 = document.querySelectorAll(".row2");
        row2 = Array.from(row2);
    let row3 = document.querySelectorAll(".row3");
        row3 = Array.from(row3);
    let row4 = document.querySelectorAll(".row4");
        row4 = Array.from(row4);
    let row5 = document.querySelectorAll(".row5");
        row5 = Array.from(row5);
    let row6 = document.querySelectorAll(".row6");
        row6 = Array.from(row6);
    let row7 = document.querySelectorAll(".row7");
        row7 = Array.from(row7);

    let player = false;
    let lastbox;

    //when the game starts its player-1's turn
    turn.textContent = "player 1";

    function checkBoard() {
        const winningArrays = [
        [0, 1, 2, 3], [41, 40, 39, 38], [7, 8, 9, 10], [34, 33, 32, 31], [14, 15, 16, 17], [27, 26, 25, 24], [21, 22, 23, 24],
        [20, 19, 18, 17], [28, 29, 30, 31], [13, 12, 11, 10], [35, 36, 37, 38], [6, 5, 4, 3], [0, 7, 14, 21], [41, 34, 27, 20],
        [1, 8, 15, 22], [40, 33, 26, 19], [2, 9, 16, 23], [39, 32, 25, 18], [3, 10, 17, 24], [38, 31, 24, 17], [4, 11, 18, 25],
        [37, 30, 23, 16], [5, 12, 19, 26], [36, 29, 22, 15], [6, 13, 20, 27], [35, 28, 21, 14], [0, 8, 16, 24], [41, 33, 25, 17],
        [7, 15, 23, 31], [34, 26, 18, 10], [14, 22, 30, 38], [27, 19, 11, 3], [35, 29, 23, 17], [6, 12, 18, 24], [28, 22, 16, 10],
        [13, 19, 25, 31], [21, 15, 9, 3], [20, 26, 32, 38], [36, 30, 24, 18], [5, 11, 17, 23], [37, 31, 25, 19], [4, 10, 16, 22],
        [2, 10, 18, 26], [39, 31, 23, 15], [1, 9, 17, 25], [40, 32, 24, 16], [9, 7, 25, 33], [8, 16, 24, 32], [11, 7, 23, 29],
        [12, 18, 24, 30], [1, 2, 3, 4], [5, 4, 3, 2], [8, 9, 10, 11], [12, 11, 10, 9], [15, 16, 17, 18], [19, 18, 17, 16],
        [22, 23, 24, 25], [26, 25, 24, 23], [29, 30, 31, 32], [33, 32, 31, 30], [36, 37, 38, 39], [40, 39, 38, 37], [7, 14, 21, 28],
        [8, 15, 22, 29], [9, 16, 23, 30], [10, 17, 24, 31], [11, 18, 25, 32], [12, 19, 26, 33], [13, 20, 27, 34]
        ];
        for(let y = 0; y < winningArrays.length; y++) {
            const square1 = squares[winningArrays[y][0]];
            const square2 = squares[winningArrays[y][1]];
            const square3 = squares[winningArrays[y][2]];
            const square4 = squares[winningArrays[y][3]];

            if(square1.classList.contains('red') &&
                square2.classList.contains('red') &&
                square3.classList.contains('red') &&
                square4.classList.contains('red')) {
                    result.innerHTML = 'Player one wins!';
                    instruct.textContent = "Press the Restart button to play again."
                    return;
            } 
            else if (square1.classList.contains('blue') &&
                square2.classList.contains('blue') &&
                square3.classList.contains('blue') &&
                square4.classList.contains('blue')) {
                    result.innerHTML = 'Player two wins!';
                    instruct.textContent = "Press the Restart button to play again."
                    return;
            }
        }
    }

    function changePlayer (square) {
        if (!player){
            square.classList.add("red");
            turn.textContent = "player 2";
        } else {
            square.classList.add("blue");
            turn.textContent = "player 1";
        }
        player = !player;

        checkBoard();
    }

    for (let i = 0;i < squares.length;i++){
        squares[i].addEventListener("click", () => {
            if (squares[i].classList.contains("red") || squares[i].classList.contains("blue") || (result.textContent != '')){
                return;
            }else if (squares[i].classList.contains("row1")){
                lastbox = row1.pop();
            }else if (squares[i].classList.contains("row2")){
                lastbox = row2.pop();
            }else if (squares[i].classList.contains("row3")){
                lastbox = row3.pop();
            }else if (squares[i].classList.contains("row4")){
                lastbox = row4.pop();
            }else if (squares[i].classList.contains("row5")){
                lastbox = row5.pop();
            }else if (squares[i].classList.contains("row6")){
                lastbox = row6.pop();
            }else if (squares[i].classList.contains("row7")){
                lastbox = row7.pop();
            }
            changePlayer(lastbox);
        });
    }
}

setGame();
